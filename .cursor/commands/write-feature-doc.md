...
Create or revise a feature document in `docs/dev/` using the canonical spec in `docs/dev/README.md`.
...
- Target file (for example `docs/dev/ui-scroll.md`)
...
1. Read `docs/dev/README.md` and apply its required section order and writing rules.
...
4. If creating a new doc:
   - Use the starter template from `docs/dev/README.md`.
   - Keep filename concise kebab-case and behavior-oriented.
5. Add/update cross-links in `## Related docs` to avoid duplicated deep detail (handoffs: `../handoffs/<file>.md` per `docs/dev/README.md`).
6. Run the authoring checklist from `docs/dev/README.md` before finalizing.
...
- Keep docs in `docs/dev/` implementation-truthful.
